...
% This isn't really what was asked for. This is a new prop record type, whereas the instructions say "anything with a Health property".
% But that's rather nebulous in erlang - there is no strict concept of a "property".
-record(prop, {
    id,
    health = 1000
}).
...
chr_status(#chr{health = 0}) -> dead;
chr_status(#chr{}) -> alive.

prop_status(#prop{health = 0}) -> destroyed;
prop_status(#prop{}) -> intact.
...
make_prop(Id) ->
    #prop{id = Id}.
make_prop(Id, Health) ->
    #prop{id = Id, health = Health}.

prop_adjust_health(Prop = #prop{health = Health}, Adjustment) ->
    Prop#prop{health = max(0, Health + Adjustment)}.
...
print_character(Chr = #chr{}) ->
    io:format("chr: ~p = ~p~n~n", [Chr, chr_status(Chr)]);
print_character(Prop = #prop{}) ->
    io:format("prop: ~p = ~p~n~n", [Prop, prop_status(Prop)]).

deal_damage(#prop{id = PropId}, To, _Distance, _Damage) ->
    io:format("prop ~p cannot deal damage~n", [PropId]),
    To;
deal_damage(From = #chr{}, To = #prop{}, Distance, Damage) ->
    deal_damage(From, To, Distance, Damage, true);
deal_damage(From = #chr{}, To = #chr{}, Distance, Damage) ->
    deal_damage(From, To, Distance, Damage, are_enemies(From, To)).

deal_damage(From = #chr{}, To, Distance, Damage, true) ->
...
deal_damage_to_enemy(From = #chr{}, To, Distance, Damage) ->
    deal_damage_to_enemy(From, To, Distance, Damage, range(From)).

deal_damage_to_enemy(#chr{name = NameFrom}, To = #prop{id = PropId}, Distance, Damage, Range) when
    Range >= Distance
->
    io:format("~p deals ~p damage to ~p~n", [NameFrom, Damage, PropId]),
    prop_adjust_health(To, -Damage);
...
deal_damage_within_range(
    From = #chr{name = NameFrom, level = LevelFrom},
    To = #chr{name = NameTo, level = LevelTo},
    Damage
) when LevelFrom =< LevelTo - 5 ->
...
deal_modified_damage(#chr{name = NameFrom}, To = #chr{name = NameTo, health = Health}, Damage) when
    Health =< Damage
->
...
heal(#prop{id = PropId}, To) ->
    io:format("prop ~p cannot peform heal~n", [PropId]),
    To;
heal(_From, To = #prop{id = PropId}) ->
    io:format("prop ~p cannot be healed~n", [PropId]),
    To;
...
run() ->
    ...
    io:format("Two solo characters (not in any faction) are enemies? = ~p~n", [
        are_enemies(Solo1, Solo2)
    ]),
    heal(Solo1, Solo2),

    % additions to cover the iteration 5 changes
    Tree = make_prop(tree, 2000),
    io:format("~nAdded a Tree prop~n"),
    print_character(Tree),
    Tree2 = deal_damage(Raith6, Tree, 1, 500),
    print_character(Tree2),
    Tree3 = heal(Raith6, Tree2),
    print_character(Tree3),
    heal(Tree2, Raith6),
    deal_damage(Tree2, Raith6, 1, 500),
    Tree4 = deal_damage(Raith6, Tree3, 1, 1600),
    print_character(Tree4),
    finished.
